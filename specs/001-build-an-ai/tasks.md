...
T001  Initialize project & install core deps
...
T002  Configure linting, formatting, and dev scripts [P]
...
T010 [P] Contract test: `POST /conversation/clarify`
...
T011 [P] Contract test: `POST /proposal/generate`
...
T012 [P] Contract test: `POST /proposal/apply`
...
T013 [P] Contract test: `POST /proposal/undo`
...
T014 [P] Contract test: `POST /tts/speak` (binary/audio response)
...
T015 [P] Contract test: `GET /calendar/events`
...
T020 [P] Integration test: Conversation clarifying flow → proposal generation
...
T021 [P] Integration test: Apply proposal to calendar (mock Google API)
...
T030 [P] Create model + Zod schema: `User` (SessionContext)
...
T031 [P] Create model + Zod schema: `CalendarEvent`
...
T032 [P] Create model + Zod schema: `ProblemStatement`, `ClarifyingQuestion`, `Proposal`, `ChangeItem`, `TranscriptEntry`, `PreferenceSet`, `SyncOperation`
...
T040 [P] Implement `lib/gemini.ts` helper (prompt + response parsing)
...
T041 [P] Implement `lib/elevenlabs-tts.ts` proxy helper
...
T042 [P] Implement `lib/proposal-schema.ts` Zod validators used in API routes
...
T060  Implement `lib/google-calendar.ts` calendar client (mockable)
