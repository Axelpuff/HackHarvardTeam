...
test.describe('Quickstart Flow', () => {
  ...
  test('should complete basic conversation and proposal flow', async ({ page }) => {
    // 1. Check that landing page shows two calendar panels
    await expect(page.locator('[data-testid="calendar-current"]')).toBeVisible();
    await expect(page.locator('[data-testid="calendar-proposed"]')).toBeVisible();

    // 2. Check for "Start Conversation" button and click it
    const startButton = page.locator('button', { hasText: 'Start Conversation' });
    ...
    const textInput = page.locator('input[placeholder*="problem"], textarea[placeholder*="problem"], input[type="text"]').first();
    ...
    await expect(page.locator('[data-testid="clarifying-question"]')).toBeVisible({ timeout: 5000 });

    // 6. Provide an answer to the clarifying question
    const answerInput = page.locator('input[placeholder*="answer"], textarea[placeholder*="answer"], input[type="text"]').first();
    ...
    await expect(page.locator('[data-testid="proposal"]')).toBeVisible({ timeout: 60000 });
  ...
  test('should persist preferences in localStorage', async ({ page }) => {
    ...
    const settingsButton = page.locator('button', { hasText: /Settings|Preferences/i }).first();
    if (await settingsButton.isVisible()) {
      await settingsButton.click();

      // Change a preference
      const sleepTarget = page.locator('input[name="sleepTargetHours"], input[placeholder*="sleep"]').first();
